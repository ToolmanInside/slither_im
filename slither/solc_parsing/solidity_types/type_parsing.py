...
def parse_type(t, caller_context):#这个t的实参是self._elem_to_parse基本类型去解析
    # local import to avoid circular dependency 
    from slither.solc_parsing.expressions.expression_parsing import parse_expression
    from slither.solc_parsing.variables.function_type_variable import FunctionTypeVariableSolc

    if isinstance(caller_context, Contract):#当是分析全局变量时他执行
        contract = caller_context
    elif isinstance(caller_context, Function):#当是分析本地变量时他执行，因为我们是从 contract.analyze_content_functions()这个地方一直看到这里的所以，我们目前是分析本地变量
    ...
    if isinstance(t, UnknownType):#UnknownType是在本文件第20行的一个类
        return _find_from_type_name(t.name, contract, contracts, structures, enums)

    elif t[key] == 'ElementaryTypeName':  #对于非紧凑型AST来说key='name'
        if is_compact_ast:
            return ElementaryType(t['name'])
        return ElementaryType(t['attributes'][key]) #对于slither SimpleDAO.sol来说t['attributes'][key]='uint'
        ...
        if is_compact_ast:
            ...
            assert len(t['children']) == 2   #name:'Mapping'的ast节点，必然有两个孩子

            mappingFrom = parse_type(t['children'][0], contract)  #递归了！
            mappingTo = parse_type(t['children'][1], contract)    #递归了！
