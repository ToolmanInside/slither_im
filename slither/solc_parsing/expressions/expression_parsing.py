...
def find_variable(var_name, caller_context, referenced_declaration=None):#var_name='msg'
    ...
    if function:
        ...
        if var_name in func_variables:#var_name='msg'
        ...
        # t(uint256)
        func_variables_ptr = {get_pointer_name(f) : f for f in function.variables}  #get_pointer_name()本文件36行，f请看下面注释
        '''
                def variables(self):
                    """
...
def parse_call(expression, caller_context): #expression:require(msg.sender.call.value(amount)());
    ...
    if caller_context.is_compact_ast:
        ...
        type_conversion = attributes['type_conversion']  #true or false
        type_return = attributes['type']  #例如"bool"

    if type_conversion:   #對於SimpleDAO.sol这一句没有通过
    ...
    if caller_context.is_compact_ast:
        ...
        children = expression['children']
        called = parse_expression(children[0], caller_context)#对于msg.sender.call.value(amount)();来说。children[0]={name:"identifier"}
...
def parse_expression(expression, caller_context):#对于变量来说expression=self._initializedNotParsed
    """
    ...
    name = expression[caller_context.get_key()]  #expression比如就可以是if(>=)
    ...
    if name == 'UnaryOperation':
        ...
        if is_compact_ast:
            ...
            attributes = expression['attributes']
        operation_type = BinaryOperationType.get_type(attributes['operator'])#例如attributes['operator']='>='
        ...
        if is_compact_ast:
            ...
            assert len(expression['children']) == 2#对啊，二元操作肯定得有2个孩子啊，expression['children'][0]是左表达式
            left_expression = parse_expression(expression['children'][0], caller_context) #注意递归了
        ...
        return parse_call(expression, caller_context)  #expression对应的是require(msg.sender.call.value(amount)())
        #parse_call(expression, caller_context)返回的是CallExpression
        ...
        if is_compact_ast:
            ...
            '''
            operation_return_type = attributes['type']  #eg: "uint256"
        ...
        if caller_context.is_compact_ast:
            ...
            value = expression['attributes']['value']   #eg:value='msg',,,value='credit'
            if 'type' in expression['attributes']:
               t = expression['attributes']['type']  #eg:t='msg',,,t='mapping(addresss=>uint(256))'

        if t:
            found = re.findall('[struct|enum|function|modifier] \(([\[\] ()a-zA-Z0-9\.,_]*)\)', t)#found=[]
            assert len(found) <= 1
            if found:
                value = value+'('+found[0]+')'
                value = filter_name(value)   #本文件206行
                #print(value)    require(bool)

        if 'referencedDeclaration' in expression:
            referenced_declaration = expression['referencedDeclaration']
        else:
            referenced_declaration = None
        var = find_variable(value, caller_context, referenced_declaration)  #本文件50行 value='msg',,,value='credit'

        identifier = Identifier(var) #var是变量对象
        return identifier

    elif name == 'IndexAccess':
        if is_compact_ast:
            index_type = expression['typeDescriptions']['typeString']  #'uint256'
            left = expression['baseExpression']
            right = expression['indexExpression']
        else:
            index_type = expression['attributes']['type']  #eg:index_type='uint256'
            children = expression['children']
            assert len(children) == 2
            left = children[0]
            right = children[1]
        left_expression = parse_expression(left, caller_context)   #递归本函数
        right_expression = parse_expression(right, caller_context) #递归本函数
        ...
        if caller_context.is_compact_ast:
            ...
            member_name = expression['attributes']['member_name']  #member='value',,,member='sender'
            member_type = expression['attributes']['type']  #"funtion(uint265) returns(funtion() payable returns(bool))",,,,,'address'
            children = expression['children']
        ...
        member_access = MemberAccess(member_name, member_type, member_expression)#形参expression
