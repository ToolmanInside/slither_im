...
class Node(SourceMapping, ChildFunction):
    ...
    def __init__(self, node_type, node_id):#node_type比如就像NodeType.ENTRYPOINT=0X0，其实就在本文件第28行
        super(Node, self).__init__()
        self._node_type = node_type
        self._sons = []
        self._fathers = []
        self._expression = None   #这个东西好好看
    ...
    def slithir_generation(self):
        if self.expression:
            ...
            '''
                ...
                credit[msg.sender] -= amount
            '''
            self._irs = convert_expression(expression, self)#把node.expression转换成node._irs
            #for ir in self._irs:
                #print(ir)
            '''
