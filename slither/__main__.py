...
def process(filename, args, detector_classes, printer_classes):
    ...
    """
    ast = '--ast-json'
    if args.compact_ast:   #如果參數是紧凑的ast
        ast = '--ast-compact-json'
    slither = Slither(filename, args.solc, args.disable_solc_warnings, args.solc_args, ast)

    return _process(filename,slither, detector_classes, printer_classes)

"""

"""
def _process(filename,slither, detector_classes, printer_classes):
    ...
    if not printer_classes: #print_classes是空
        logger.info("========mml========"+filename)
...
def get_detectors_and_printers():
    ...
    detectors = [
        ...
        # # ConstantPragma,   不同pragma版本来回随便用
        # OldSolc,
        # Reentrancy,
        # LockedEther,
        # ArbitrarySend,
        ...
        # Assembly,
        # LowLevelCalls,
        # #NamingConvention,  例如没有按照驼峰命名法
        # ConstCandidateStateVars,
        # #ComplexFunction,
        # UnusedReturnValues,
        # #ExternalFunction,   public functions that are never called by the contract should be declared external to save gas.
    ...
    from slither.printers.summary.signature_abstraction import SignatureAbstraction
...
def main():
    '''
    通过本文件的get_detectors_and_printers()这个方法得到detectorsO(列表),printers(列表)。因为有这两个列表才可以生成args对象
    :return:
    '''
    detectors, printers = get_detectors_and_printers()  # 得到检测者和打印者

    main_impl(all_detector_classes=detectors, all_printer_classes=printers)


def main_impl(all_detector_classes, all_printer_classes):  #主实现
    """
    ...
    detector_classes = choose_detectors(args, all_detector_classes) #根据參數来挑选出dectector
    printer_classes = choose_printers(args, all_printer_classes)    #根据參數来挑选出printer

    default_log = logging.INFO if not args.debug else logging.DEBUG  #args.debug为空则logging.INFO
    ...
    try:
        ...
        if os.path.isfile(filename):
            (results, number_contracts) = process(filename, args, detector_classes, printer_classes)#:param detctor_classes是已经筛选过的
            ...
            extension = "*.sol" if not args.solc_ast else "*.json"  # args.solc_ast是空extension="*.sol"
        ...
        if printer_classes:
            ...
            logger.info('%s analyzed (%d contracts), %d result(s) found\nINFO:Slither:========mml========', filename, number_contracts, len(results))
        exit(results)
...
def parse_args(detector_classes, printer_classes):
    """
    detector_classes，printer_classes列表，解析命令行參數
    """
...
class ListDetectors(argparse.Action):   #列出所有detector
    def __call__(self, parser, *args, **kwargs):
        detectors, _ = get_detectors_and_printers()
        output_detectors(detectors)
        parser.exit()

class ListPrinters(argparse.Action):  #列出所有printer
    def __call__(self, parser, *args, **kwargs):
...
def choose_detectors(args, all_detector_classes):
    ...
    detectors_to_run = []      #要运行的dectors都存这里
